// All four product screens share one backend forecast and charging scenario.
const modelState = {
    data: null,
    loading: true,
    error: "",
...
function modelTime(value, date = false) {
...
function selectedPrediction() {
    return modelState.data.predictions.find(
        (p) => p.horizonMinutes === modelState.horizon,
    );
}
...
function modelHeader(page) {
    return `<header class="page-head"><div><h1>${{ overview: "Overview", forecast: "Forecast", charging: "Charging", impact: "Impact" }[page]}</h1><p>Irish renewable dispatch-down and potential flexible-load recovery</p></div><div class="status model-status"><span class="status-meta">${modelState.data ? "Fetched " + escapeHtml(modelTime(modelState.data.generatedAt, true)) : ""}</span><span class="status-ready">${modelState.loading ? "Loading model…" : modelState.error ? "Backend unavailable" : dataSourceLabel()}</span></div></header>`;
}
function modelCell(label, value, caption = "", color = "") {
    return `<div class="summary-cell"><div class="metric-label">${label}</div><div class="mid-value ${color}">${value}</div><div class="metric-caption">${caption}</div></div>`;
}
function modelCause(p) {
    if (p.atRiskMwh === 0) return "No predicted dispatch-down";
    return p.constraintMwh === p.curtailmentMwh
        ? "Mixed components"
        : p.constraintMwh > p.curtailmentMwh
          ? "Constraint-dominated"
          : "Curtailment-dominated";
}
function modelChart(forecast) {
    ...
    const ticks = [0, 0.25, 0.5, 0.75, 1]
        .map(
            (f) =>
                `<line x1="65" y1="${y(max * f)}" x2="740" y2="${y(max * f)}" stroke="#dfe7f2"/><text x="55" y="${y(max * f) + 5}" text-anchor="end" fill="#52617f" font-size="14">${modelNumber(max * f)}</text>`,
        )
        .join("");
    return `<svg viewBox="0 0 800 340" role="img" aria-label="${forecast ? "Energy at risk with P10 to P90 intervals" : "Energy at risk and potential recovery"} for 30 and 60 minute horizons"><text x="15" y="25" fill="#52617f">MWh per half-hour</text>${ticks}${points
        .map((p, i) => {
            const x = 270 + i * 310;
            return `<rect x="${x - 65}" y="${y(p.atRiskMwh)}" width="55" height="${270 - y(p.atRiskMwh)}" fill="#ed4249" opacity=".8"/>${forecast ? `<line x1="${x + 20}" y1="${y(p.upperMwh)}" x2="${x + 20}" y2="${y(p.lowerMwh)}" stroke="#b52643" stroke-width="3"/><path d="M ${x + 8} ${y(p.upperMwh)} h24 M ${x + 8} ${y(p.lowerMwh)} h24" stroke="#b52643" stroke-width="3"/><circle cx="${x + 20}" cy="${y(p.medianMwh)}" r="5" fill="#b52643"/>` : `<rect x="${x + 5}" y="${y(scenarioRecovery(p))}" width="55" height="${270 - y(scenarioRecovery(p))}" fill="#078c58"/>`}<text x="${x}" y="300" text-anchor="middle" fill="#52617f">+${p.horizonMinutes} min · ${escapeHtml(modelTime(p.targetAt))}</text><text x="${x}" y="324" text-anchor="middle" fill="#52617f" font-size="13">${modelNumber(p.atRiskMwh)} MWh at risk</text>`;
        })
        .join("")}</svg>`;
}
function modelView(page) {
    ...
    if (modelState.loading)
        return (
            top +
            '<section class="card model-message" role="status">Loading predictions from GridToEv…</section>'
        );
    if (modelState.error)
        return (
            top +
            `<section class="card model-message" role="alert"><h2>Forecast unavailable</h2><p>${escapeHtml(modelState.error)}</p><p>Run <code>python backend/server.py</code> and open the app at <code>http://127.0.0.1:8080</code>. The backend must be running; it can supply demo data if the model is unavailable.</p><button class="primary-button" id="model-retry">Try again</button></section>`
        );
    const d = modelState.data,
        p = selectedPrediction(),
        forecast = page === "forecast";
    const provenance = `<p class="model-provenance">${dataSourceLabel()} · issued ${escapeHtml(modelTime(p.issuedAt, true))} · ${escapeHtml(settings.timezone)} · model ${escapeHtml(d.modelVersion)}. Recovery uses ${modelNumber(d.flexibleCapacityMw)} MW and ${modelNumber(d.scenario.flexibleDemandKwh)} kWh flexible demand. Projected scenario ${escapeHtml(d.scenario.id)}, not measured recovery.</p>`;
    const time = modelCell(
        "Forecast target",
        escapeHtml(modelTime(p.targetAt)),
        `+${p.horizonMinutes} minutes from issue time`,
    );
    const summary = forecast
        ? time +
          modelCell(
              "Dispatch-down probability",
              modelNumber(p.probability * 100) + "%",
              "Model event probability",
              "red",
          ) +
          modelCell(
              "Expected energy at risk",
              modelNumber(p.atRiskMwh) + " MWh",
              "Per half-hour",
              "red",
          ) +
          modelCell(
              "P10–P90 range",
              `${modelNumber(p.lowerMwh)}–${modelNumber(p.upperMwh)} MWh`,
              "Model uncertainty range",
          ) +
          modelCell("Predicted component", modelCause(p))
        : time +
          modelCell(
              "Energy at risk",
              modelNumber(p.atRiskMwh) + " MWh",
          ...
          modelCell(
              "Scenario capacity",
              modelNumber(d.flexibleCapacityMw * 0.5) + " MWh",
              `${modelNumber(d.flexibleCapacityMw)} MW × 0.5 hours`,
          ) +
          modelCell(
              "Potential recovery",
              modelNumber(scenarioRecovery(p)) + " MWh",
              "Demand and power limited",
              "green",
          ) +
          modelCell("Dispatch-down risk", escapeHtml(p.risk), modelCause(p));
    const chartCard = `<section class="card chart-card"><h2 class="card-title">${forecast ? "Short-term dispatch-down forecast" : "At-risk energy and potential recovery"}</h2><p class="card-subtitle">Two forecast horizons from the same historical issue time</p><div class="chart-holder">${modelChart(forecast)}</div>${legend(
        forecast
            ? [
                  ["red", "Energy at risk"],
                  ["pink", "P10–P90 range; dot = P50"],
              ]
            : [
                  ["red", "Energy at risk"],
    ...
    const shares = [
        ["Constraint", p.constraintMwh],
        ["Curtailment", p.curtailmentMwh],
    ]
        .map(([name, value]) => {
            const share = p.atRiskMwh > 0 ? (value / p.atRiskMwh) * 100 : 0;
            return `<div class="bar-row"><span>${name}</span><div class="bar-track"><div class="bar-fill" style="width:${share}%"></div></div><span>${p.atRiskMwh > 0 ? modelNumber(share) + "%" : "N/A"}</span></div><p class="metric-caption">${modelNumber(value)} MWh</p>`;
        })
        .join("");
...
async function loadModelForecast() {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(
                body.error?.message || "Unable to fetch predictions.",
            );
        if (
            !Array.isArray(body.predictions) ||
            body.predictions.length !== 2 ||
            !body.scenario ||
            !Array.isArray(body.scenario.outcomes) ||
...
document.addEventListener("change", (event) => {
    if (event.target.id === "model-horizon") {
        modelState.horizon = Number(event.target.value);
        render();
    }
});
